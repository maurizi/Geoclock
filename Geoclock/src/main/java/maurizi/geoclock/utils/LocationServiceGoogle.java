...
public class LocationServiceGoogle {
  ...
  public Task<Void> removeGeofence(@NonNull GeoAlarm alarm) {
    return geofencingClient.removeGeofences(Collections.singletonList(alarm.id.toString()));
  }
  ...
  private Geofence buildGeofence(GeoAlarm alarm) {
    float radius =
        Math.max(
            1f, alarm.radius); // setCircularRegion throws IllegalArgumentException if radius <= 0
