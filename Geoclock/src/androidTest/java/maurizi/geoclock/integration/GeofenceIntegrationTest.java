 ...
 *
 * These tests require a google_apis emulator image (Play Services). They are annotated @LargeTest
 * because geofence evaluation by Google Play Services can take up to 30 seconds.
 *
 * If flaky in CI, annotate with @RequiresDevice and exclude from automated runs using:
 *   gradlew connectedAndroidTest -Pandroid.testInstrumentationRunnerArguments.notAnnotation=...
 */
@RunWith(AndroidJUnit4.class)
@LargeTest
public class GeofenceIntegrationTest {

    private static final long GEOFENCE_TIMEOUT_MS = 30_000L;
    ...
    public void setUp() throws Exception {
        ...
        fusedClient.setMockMode(true)
                .addOnSuccessListener(v -> { mockModeOk.set(true); latch.countDown(); })
    ...
    private void assumePlayServices() {
        ...
        Assume.assumeTrue("Google Play Services not available; geofence transitions require it",
    ...
    public void enterGeofence_startsAlarmRingingService() throws Exception {
        ...
        assertNotNull("Geofence ENTER should schedule an alarm clock",
    ...
    public void exitGeofence_removesFromActiveAlarms() throws Exception {
        ...
        assertNull("Geofence EXIT should clear active alarm from alarm manager",
                alarmManager.getNextAlarmClock());
    }
    ...
    public void disabledAlarm_geofenceEnter_doesNotStartService() throws Exception {
        ...
        AlarmManager alarmManager = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        assertNull("Disabled alarm geofence ENTER should not schedule alarm clock",
                alarmManager.getNextAlarmClock());
    }
