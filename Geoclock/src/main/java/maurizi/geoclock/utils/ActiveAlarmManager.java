...
public class ActiveAlarmManager {
    ...
    return activeAlarms.stream()
        ...
        .min(
            Comparator.comparingLong(
                alarm -> alarm.calculateAlarmTime(now).toInstant().toEpochMilli()))
        .orElse(null);
  }
  ...
  public void addActiveAlarms(ImmutableSet<UUID> triggerAlarmIds) {
    ...
    final ImmutableSet<UUID> currentAlarms =
        ImmutableSet.copyOf(Sets.union(savedAlarmIds, triggerAlarmIds));
  ...
  public void removeActiveAlarms(Set<UUID> triggerAlarmIds) {
    final Set<UUID> savedAlarmIds = getSavedAlarms();
    final ImmutableSet<UUID> currentAlarms =
        ImmutableSet.copyOf(Sets.difference(savedAlarmIds, triggerAlarmIds));
    changeActiveAlarms(currentAlarms);
  }
  ...
  private void changeActiveAlarms(Set<UUID> currentAlarmIds) {
    activeAlarmsPrefs
        .edit()
        .putString(ACTIVE_ALARM_IDS, gson.toJson(currentAlarmIds.toArray(), UUID[].class))
        .apply();
    Set<GeoAlarm> currentAlarms =
        filter(
            newHashSet(GeoAlarm.getGeoAlarms(context)),
            alarm -> currentAlarmIds.contains(alarm.id));
  ...
  private Set<UUID> getSavedAlarms() {
    final String savedActiveAlarmsJson =
        activeAlarmsPrefs.getString(ACTIVE_ALARM_IDS, gson.toJson(new UUID[] {}));
  ...
  private void setNotification(@NonNull final GeoAlarm alarm, final ZonedDateTime alarmTime) {
    ...
    if (notificationTime.isBefore(now)) {
      ...
      final PendingIntent pendingNotificationIntent =
          NotificationReceiver.getPendingIntent(context, alarm);
