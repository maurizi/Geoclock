 ...
 *
 * <p>Uses getActiveNotifications() + PendingIntent.send() rather than notification-shade UI
...
public class NotificationActionsTest {
  ...
  public void snoozeAction_notificationButton_schedulesSnooze() throws Exception {
    ...
    Notification.Action snoozeAction =
        findRingingAction(context.getString(maurizi.geoclock.R.string.snooze_5min));
    assertNotNull("Snooze action should be in ringing notification", snoozeAction);

    snoozeAction.actionIntent.send(context, 0, null);
    Thread.sleep(1000);

    assertNotNull("Snooze should schedule an alarm clock", alarmManager.getNextAlarmClock());
  }
  ...
  public void dismissAction_notificationButton_stopsService() throws Exception {
    ...
    Notification.Action dismissAction =
        findRingingAction(context.getString(maurizi.geoclock.R.string.dismiss));
    ...
    assertNull(
        "Ringing notification should be gone after dismiss",
  ...
  public void cancelAlarm_upcomingNotification_disablesAlarm() throws Exception {
    ...
    Notification.Action cancelAction =
        findAction(
            ...
            context.getString(maurizi.geoclock.R.string.cancel_alarm));
  ...
  /**
   * Starts the ringing service via AlarmClockReceiver broadcast (grants foreground-service start
   * privilege).
   */
