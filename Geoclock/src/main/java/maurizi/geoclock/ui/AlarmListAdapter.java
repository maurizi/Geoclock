...
public class AlarmListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	static final EnumSet<DayOfWeek> WEEKDAYS = EnumSet.of(
	        DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY,
	        DayOfWeek.THURSDAY, DayOfWeek.FRIDAY);
	static final EnumSet<DayOfWeek> WEEKENDS = EnumSet.of(
	...
	void sortAlarms() {
		...
		Collections.sort(alarms, Comparator
		        .<GeoAlarm, Boolean>comparing(a -> !isInsideGeofence(a, loc)) // inside first (false < true → negate)
		        .thenComparing(a -> {
	...
	public void onBindViewHolder(@NonNull RecyclerView.ViewHolder vh, int position) {
		...
		int bellColor = inside
		        ? ContextCompat.getColor(ctx, R.color.bell_inside)
		        : ContextCompat.getColor(ctx, R.color.bell_outside);
		android.graphics.drawable.Drawable icon = DrawableCompat.wrap(holder.bellIcon.getDrawable()).mutate();
