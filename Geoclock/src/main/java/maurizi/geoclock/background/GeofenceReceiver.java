...
public class GeofenceReceiver extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (null != affectedGeofences && !affectedGeofences.isEmpty()) {
      final Collection<GeoAlarm> affectedAlarms =
          filter(
              Lists.transform(affectedGeofences, getGeoAlarmForGeofenceFn(context)),
              a -> a != null);
      ImmutableSet<UUID> affectedAlarmIds =
          ImmutableSet.copyOf(transform(affectedAlarms, alarm -> alarm.id));
      if (transition == Geofence.GEOFENCE_TRANSITION_ENTER) {
        ImmutableSet<UUID> enabledAlarmIds =
            ImmutableSet.copyOf(
