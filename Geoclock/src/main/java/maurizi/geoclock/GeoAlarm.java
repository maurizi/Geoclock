...
public class GeoAlarm {
  ...
  public ZonedDateTime calculateAlarmTime(LocalDateTime now) {
    ...
    final LocalDateTime alarmDateTime =
        isNonRepeating()
            ? alarmTime.atDate(
                isAlarmForToday(now) ? now.toLocalDate() : now.toLocalDate().plusDays(1))
  ...
  private LocalDate getSoonestDayForRepeatingAlarm(LocalDateTime now) {
    ...
    final Collection<DayOfWeek> daysAfterToday =
        filter(days, weekday -> weekday.getValue() > currentDayOfWeek.getValue());

    final Collection<DayOfWeek> daysTodayAndBefore =
        filter(days, weekday -> weekday.getValue() <= currentDayOfWeek.getValue());

    final ImmutableList<DayOfWeek> allDays =
        ImmutableList.<DayOfWeek>builder()
            .addAll(daysAfterToday)
            .addAll(daysTodayAndBefore)
            .build();
  ...
  public static Function<com.google.android.gms.location.Geofence, GeoAlarm>
      getGeoAlarmForGeofenceFn(Context context) {
  ...
  public static void save(Context context, GeoAlarm newAlarm) {
    ...
    editor.putString(newAlarm.id.toString(), gson.toJson(newAlarm, GeoAlarm.class)).apply();
  }
